...
void Qst_GetQstFromForm (struct Qst_Question *Question)
  {
   ...
   Med_GetMediaFromForm (Gbl.Hierarchy.Node[HieLvl_CRS].HieCod,Question->QstCod,
...
void Qst_MoveMediaToDefinitiveDirectories (struct Qst_Question *Question)
  {
   ...
   CurrentMedCodInDB = Qst_GetMedCodFromDB (Gbl.Hierarchy.Node[HieLvl_CRS].HieCod,Question->QstCod,
...
void Qst_RemoveSelectedQsts (void)
  {
   ...
   if (Tst_GetParsTst (&Questions,Tst_EDIT_QUESTIONS))	// Get parameters
	   ...
	   NumQst++)
	{
	 ...
	 Qst_RemoveOneQstFromDB (Gbl.Hierarchy.Node[HieLvl_CRS].HieCod,QstCod);
	}
   ...
   if (Question->QstCod > 0)
     {
      ...
      Tag_DB_RemoveUnusedTagsFromCrs (Gbl.Hierarchy.Node[HieLvl_CRS].HieCod);
