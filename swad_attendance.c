...
static void Att_ListEventStudents (struct Att_Events *Events)
  {
   ...
   Box_BoxBegin (Txt_Attendance,NULL,NULL,
      ...
      Grp_ShowFormToSelectSeveralGroups (ActSeeOneAtt,Att_PutParAttCod,Events,
					 NULL);
	 ...
	 if (Gbl.Usrs.LstUsrs[Rol_STD].NumUsrs)
	   {
	    /***** Get my preference about photos
	           in users' list for current course *****/
	    ShowPhotos = Pho_GetMyPrefAboutListWithPhotosFromDB ();
   ...
   if (ListCods->NumUsrs)
     {
      ...
      Usr_CreateSubqueryUsrCods (ListCods,&SubQueryUsrs);
...
static void Att_PutFormToPrintMyListPars (void *Events)
  {
   if (Events)
     {
      if (((struct Att_Events *) Events)->ShowDetails == Lay_SHOW)
...
static void Att_PutIconToViewAttEvents (__attribute__((unused)) void *Args)
  {
   Ico_PutContextualIconToViewAttEvents (ActSeeAllAtt,NULL,NULL);
  }
   ...
   for (NumAttEvent = 0, UniqueId = 1;
      ...
      if (Events->Lst[NumAttEvent].Checked == HTM_CHECKED)
	{
	 ...
	 Present = Att_CheckIfUsrIsPresentInEventAndGetComments (Events->Event.AttCod,UsrDat->UsrCod,
								 CommentStd,CommentTch);
         ShowComment.Std = CommentStd[0] ? Lay_SHOW :
					   Lay_DONT_SHOW;
	 ShowComment.Tch = CommentTch[0] &&
	                   (Gbl.Usrs.Me.Role.Logged == Rol_TCH ||
	                    Events->Event.CommentTchVisible == HidVis_VISIBLE) ? Lay_SHOW :
	                							 Lay_DONT_SHOW;
	 ...
	 if (ShowComment.Std == Lay_SHOW ||
	     ShowComment.Tch == Lay_SHOW)
	   {
	    HTM_TR_Begin (NULL);
