...
static void Enr_ShowFormRequestNewAccountWithParams (const char *NewNicknameWithoutArroba,
                                                     const char *NewEmail);
static bool Enr_GetParamsNewAccount (char *NewNicknameWithoutArroba,
                                     char *NewEmail,
                                     char *NewEncryptedPassword);
static void Enr_PrintAccountSeparator (void);
...
static void Enr_AskIfCompletelyEliminateUsrFromPlatform (bool ItsMe);
...
bool Enr_CreateNewAccountAndLogIn (void)
  {
   ...
   if (Enr_GetParamsNewAccount (NewNicknameWithoutArroba,NewEmail,NewEncryptedPassword))
     {
      /***** User's has no ID *****/
      ...
      Enr_CreateNewUsr (&Gbl.Usrs.Me.UsrDat);
      ...
      if (Mai_UpdateEmailInDB (&Gbl.Usrs.Me.UsrDat,NewEmail))
	{
     ...
     {
      /***** Show form again ******/
      Enr_ShowFormRequestNewAccountWithParams (NewNicknameWithoutArroba,NewEmail);
      return false;
     }
  }
...
void Enr_ShowFormChangeMyAccount (void)
  {
   ...
   if (Enr_CheckIfICanEliminateAccount (true))	// ItsMe = true
      Enr_PutLinkToRemoveMyAccount ();
      ...
      for (NumUsr = 0;
           ...
           NumUsr++)
        {
         ...
         if (Usr_ChkUsrCodAndGetAllUsrDataFromUsrCod (&UsrDat))        // If user's data exist...
           {
            Enr_DeleteUsrFromPlatform (&UsrDat,Cns_QUIET);
            NumUsrsEliminated++;
           }
        }
...
void Enr_CreatAndShowNewUsrRecordAndRegInCrs (void)
  {
   ...
   if (ID_CheckIfUsrIDIsValid (Gbl.Usrs.Other.UsrDat.IDs.List[0].ID))        // User's ID valid
	 ...
	 Lay_ShowErrorAndExit ("A teacher only can create new users as students.");
      ...
      Enr_CreateNewUsr (&Gbl.Usrs.Other.UsrDat);
