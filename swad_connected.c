...
void Con_RefreshConnected (void)
  {
   unsigned NumUsr;
   Lay_Show_t ShowConnected = (Gbl.Prefs.SideCols & Lay_SHOW_RIGHT_COLUMN) != 0 &&	// Right column visible
                              Gbl.Hierarchy.HieLvl == Hie_CRS ? Lay_SHOW :		// There is a course selected
                        					Lay_DONT_SHOW;
...
void Con_ShowGlobalConnectedUsrs (void)
  {
   ...
   if (NumUsrsTotal)
	 ...
	 /***** Write total number of users *****/
         HTM_UnsignedTxt (NumUsrsTotal,
                          Txt_user[Usr_SEX_UNKNOWN],Txt_users[Usr_SEX_UNKNOWN]);
         HTM_Colon ();
								      ...
								      Rol_Role_t Role)
  {
   ...
   if (Usrs.NumUsrs)
	 ...
	 HTM_TD_Begin ("colspan=\"3\" class=\"CT\"");
	    HTM_UnsignedTxt (Usrs.NumUsrs,
			     Txt_ROLES_SINGUL_abc[Role][Usrs.Sex],
...
static void Con_ShowConnectedUsrsWithARoleBelongingToCurrentCrsOnRightColumn (Rol_Role_t Role,unsigned *NumUsr)
  {
   ...
   if (NumUsrsThisRole)
	 ...
	 HTM_TD_Begin ("colspan=\"3\" class=\"CON_USR_NARROW_TIT\"");
	    HTM_UnsignedTxt (NumUsrsThisRole,
		             Txt_ROLES_SINGUL_abc[Role][UsrSex],
