...
#define Cht_MAX_BYTES_ROOM_SHRT_NAME	((Cht_MAX_CHARS_ROOM_SHRT_NAME + 1) * Cns_MAX_BYTES_PER_CHAR - 1)	// 2047

#define Cht_MAX_CHARS_ROOM_FULL_NAME	(256 - 1)	// 255, maximum number of chars of the full name of a chat room
#define Cht_MAX_BYTES_ROOM_FULL_NAME	((Cht_MAX_CHARS_ROOM_FULL_NAME + 1) * Cns_MAX_BYTES_PER_CHAR - 1)	// 4095
...
static void Cht_WriteLinkToChat1 (const char *RoomCode,const char *RoomShrtName,const char *RoomFullName,
                                  unsigned Level,Lay_Last_t IsLastItemInLevel[1 + Cht_CHAT_MAX_LEVELS])
  {
   ...
   HTM_LI_Begin (NULL);
	 ...
	 /* Begin button */
	 HTM_BUTTON_Submit_Begin (RoomFullName,NULL,
